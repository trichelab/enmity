...
test:
	...
	R --slave -e 'Sys.setenv(NOT_CRAN="true"); library(testthat); setwd(file.path(.libPaths()[1], "enmity", "tests")); system.time(test_check("enmity", filter="${file}", reporter=ifelse(nchar("${r}"), "${r}", "summary")))'
...
check: build
	-R CMD check --as-cran enmity_$(VERSION).tar.gz
	rm -rf enmity.Rcheck/
